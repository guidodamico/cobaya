...
class challengeA(Likelihood_eft):
    ...
    def init_cache(self, n=5):
        """
        Set how many states to cache
        """
        self.thecache = deque(maxlen=n)
    ...
    def cache_or_compute(self, **params_values):
        ...
        """
        mypars = tuple([self.theory.get_param(par) for par in
        ["As", "H0", "omegach2", "omegabh2", "ns"]])
        cached = False
        for mycache in self.thecache:
            if mypars in mycache.keys():
                self.log.debug("Re-using computed results for likelihood")
                self.bird = mycache[mypars]
                cached = True
                break
        if not cached:
            self.log.debug("Computing new state for likelihood")
            ...
            self.thecache.appendleft({mypars: self.bird})
        return self.bird
    ...
    def logp(self, **params_values):
        ...
        if self.birdlkl is 'fastmarg' or self.birdlkl is 'fastfull':
            ...
            # else:
            #     self.projection.kdata(self.bird)
            self.bird = self.cache_or_compute(**params_values)
